...
myStartupHook = do
 -- spawn "/usr/bin/stalonetray"
  ...
  spawn "pacmd set-card-profile 0 output:analog-stereo+input:mono-fallback"
  spawn "pacmd set-card-profile 1 off"
  spawn "pacmd set-card-profile 2 off"
  spawn "pacmd set-card-profile 3 off"
